    ...
    B::Error: Into<BoxError> + 'static,
{
    ...
    fn poll(
        ...
        let Some(returned) = this.waiting.as_ref() else {
        ...
        // 4. A closed connection must not be returned to the pool.
        if returned.http_sender.is_closed() {
            tracing::trace!("HTTP/1 sender closed before becoming reusable");
            return Poll::Ready(());
        }
        ...
        // 6. Once truly ready, create the next lease and return it to the pool.
        let returned = this
            .waiting
            .take()
        ...
        // 7. Re-register the receiver waker for the next return or for the last
        // tx being dropped.
        match this.return_rx.poll_recv(cx) {
            Poll::Ready(Some(returned)) => {
